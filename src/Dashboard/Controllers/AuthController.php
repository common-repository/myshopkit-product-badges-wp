...
class AuthController
{
	...
	public function __construct()
	{
		...
		add_action('wp_ajax_' . $this->purchaseCodeAction, [$this, 'ajaxSavePurchaseCode']);
	}
	...
	public function ajaxSavePurchaseCode()
	{
		...
		if (isset($_POST['params']) && !empty($_POST['params']) && isset($_POST['params']['purchase_code'])) {
			update_option(
				MYSHOPKIT_MB_WP_PREFIX . 'purchase_code',
				sanitize_text_field($_POST['params']['purchase_code'])
			);
		}
	...
	public function getCodeAuth()
	{
		try {
			...
			if (!$oAuthenticated instanceof WP_User) {
				throw new Exception(
					esc_html__('Invalid Application Username or Token', 'myshopkit-product-badges-wp'), 400
				);
			}
	...
	public function registerMenu()
	{
		add_submenu_page(
			...
			esc_html__('Auth Settings', 'myshopkit-product-badges-wp'),
			esc_html__('Auth Settings', 'myshopkit-product-badges-wp'),
			'administrator',
			$this->getAuthSlug(),
			[$this, 'renderSettings']
